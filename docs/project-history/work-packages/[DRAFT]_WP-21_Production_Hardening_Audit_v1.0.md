...
- **P3:** WP-20 archive backup retention policy; verify `constraints.ts` path referenced by the Founder Decision Register still exists; confirm no caller still uses the raw (unverified) JWT parse path from WP-8B; deeper dependency-graph dead-code pass; full documentation-vs-code drift audit (both out of this session's budget).

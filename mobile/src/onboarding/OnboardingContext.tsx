...
export function OnboardingProvider({
  children,
  // Keys the persisted blob per signed-in user when available. Falls back to a single
  // device-local key (documented stopgap, see header comment) if no id is supplied — the
  // onboarding flow is only ever reachable while signed in (see (onboarding)/_layout.tsx's guard),
  // so callers should generally pass the session's user id.
  userId,
}: {

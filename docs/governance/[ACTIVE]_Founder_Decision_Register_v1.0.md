...
**[FD-07 update, 2026-07-17 — Wave 1.5/2a investigation]** Confirmed: the source workbook (`Indian_Meal_Cohort_Persona_DB_v3.xlsx`) contains **no** cohort-class acceptance-rate data anywhere — not a partial set, a full-text search of every sheet found only qualitative "safe prior" design notes, never numeric per-(cohort,class) values, and the workbook's own `DB_Implementation_v3` sheet (its authored list of the 11 tables it supports) does not include `re_cohort_class_priors` at all. `database/etl/generate_re_seeds.py` correctly reflects this — it has no code path for this table. **This is therefore not an ETL bug and not a partial-coverage gap to backfill; it is a research-commissioning gap.**
...
### FD-15 — Planning Semantics Architecture direction (persona/segment layer redesign)
- **Status:** Ratified (2026-07-17, claude.ai Planning Semantics session) · **Origin:** `[ACTIVE]_Canonical_Planning_Semantics_Architecture_v1.0.md`, `[ACTIVE]_Phase1_Persona_Decomposition_Catalog_v1.0.md`, `[ACTIVE]_Phase1B_Attribute_to_Class_Rule_Extraction_v1.0.md`, `[ACTIVE]_Canonical_Planning_Model_v1.0.md` (all `docs/architecture/`).
- **Context:** The 41-row `Persona_Master_v3` catalog was found to conflate two different things: a genuine, irreducible household-composition fact (who lives here, what region) and a set of independently-firing planning conditions (toddler, diabetic, fitness, budget, etc.) that had each collapsed to a single row regardless of what else varied. `household_members.segment` (currently 8 hardcoded values, per WP-8E §item 6's note that `re_segment_addon_rule` does not exist as a table and the real add-on path — `re_household_addon_plans` + `re_addon_classes` + `re_addon_dish_options` — needs the documentation/schema mismatch reconciled before addon slots are generated) cannot carry the real condition vocabulary or combine multiple conditions per member.
- **Problem Statement:** Should the persona/segment layer be preserved as-is (a prior investigation's initial position), or split into a small composition catalog plus an independent, combinable, genome-space condition/attribute inference layer with three-channel (absorb/swap/add) routing?
- **Alternatives Considered:** (a) Preserve the 41-row catalog unchanged, as the original investigation first concluded; (b) Split into composition catalog (kept, ~5 archetypes) + rules-based planning-semantics layer mapping conditions → shared genome-space attributes → three-channel routing, as revised and stress-tested across four novel households in the Canonical Planning Model.
- **Final Decision:** **Ratified — Option (b).** This explicitly **supersedes** the flat 41-persona / 8-segment vocabulary approach for the add-on layer. All 41 `Persona_Master_v3` rows were classified (Phase1 Catalog): ~8 composition rows (~5 archetypes + variants), ~22 condition-driven rows, ~7 diet-dimension rows, 2 region-axis rows, 1 compound patch (`P41`, dissolves entirely). The 92 boost-class lists were collapsed into 8 shared attribute→class rules (Rules A–H) plus ~12 condition specializations (Phase1B). The reasoning model's shared-meal-preservation principle and three-channel routing (absorb/swap/add — the swap channel recovered from P41's own text, missing from the architecture doc's first draft) were stress-tested against four households never in `Persona_Master_v3` and held stable (Canonical Planning Model §7).
- **Business Rationale:** A household with a new, never-seen combination of conditions (e.g., teenager + diabetic grandmother + fitness-conscious father) no longer needs a new persona row invented for it — every condition it contains already has its own independent rule. This is the structural precondition for LF-C (member add-ons), which DOC-03 calls "the differentiator no competitor has built."
- **Technical Rationale:** The RE's core scoring formula (CohortPrior/ContentMatch/PersonalHistory/ContextFit/ExplorationBonus) and hard-constraint/safety-gate discipline are structurally unchanged — this adds a new input to `ContentMatch`/candidate filtering, it does not redesign the scoring pipeline. Semantic attributes deliberately mirror existing Food DNA dimensions (RE-DOC-02), so the RE consumes derived implications natively with no new scoring mechanism.
- **Engineering Impact:** Resolves the `household_members.segment` vocabulary blocker by direction: **Phase 2 (vocabulary fix)** — resolving `household_members.segment` to the real condition vocabulary and allowing multiple tags per member — is now the specified path, to be built as the next concrete engineering step. FD-06's separate question (LF-C build-order *sequencing* relative to other Epics) remains independently Pending — this decision fixes *what* LF-C should be built against, not *when*.
- **AI Impact:** None directly. **Recommendation Engine Impact:** LF-C01/C02 (member add-ons) should be designed against this rules layer from the start (Phase 3), not the old 20-value single-select vocabulary — nothing needs to be un-built, since LF-C was never implemented. **Database Impact:** None yet — Phase 2's `household_members.segment` vocabulary fix is a future schema change, not made by this decision itself. **API Impact:** None yet. **Batch/ETL Impact:** None. **Architecture Impact:** Persona/cohort catalog narrows in responsibility (composition + cold-start prior only); does not eliminate cohorts/personas as a concept.
- **Affected Documents:** `[ACTIVE]_DOC-P3-02_Conceptual_Domain_Model_v1.1.md` (persona/segment sections flagged superseded on this topic pending its own future revision — see cross-reference added §Entity 3).
- **Affected Work Packages:** None yet opened for Phase 2–4 — tracked as future work, sequenced per the Migration Path (`Canonical_Planning_Semantics_Architecture_v1.0.md` §7).
- **Affected Tests:** None yet — no code or schema changed by this decision.
- **Affected Epics:** Epic 6 (Member Add-ons, closes LF-C) — its design input changes from the old segment vocabulary to this rules layer.
- **Implementation Notes:** Phase 2–4 sequencing (fix vocabulary → build LF-C against new rules → shrink persona catalog) is deliberately ordered so cold-start behavior, which depends on the composition catalog, is never disrupted while the condition layer is rebuilt underneath it.
- **Acceptance Criteria:** This decision is satisfied by its own ratification — no further sign-off gate blocks Phase 2 from starting. Two open questions remain genuinely open per the architecture doc §8: exact composition-archetype count (Phase1 Catalog gives an initial answer of ~5, flagged as needing a full confirmatory pass) and where cooking-capability rules attach (resolved as household-scoped by Phase1 Catalog §4, per the data).

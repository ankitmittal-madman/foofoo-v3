...
def client(scopes=frozenset({"ontology:read", "ontology:write", "ontology:admin"})):
    settings = Settings(environment="test", database_url=None,
                        principals=(Principal("test", TOKEN, scopes),))
...
def dish_payload(name="Masala Dosa", role="primary", class_code="BF_CRISP_SAVOURY"):
    return {
        ...
        "fields": {"cuisine": {"value": "south_indian", "confidence": 0.95,
                                 "review_status": "accepted", "evidence": EVIDENCE}},
    }


def create_dish(api, payload=None, key="dish-1"):
    return api.post("/v1/dishes", json=payload or dish_payload(),
...
def test_enrichment_is_async_and_deduplicated():
    ...
    result = api.post(f"/v1/dishes/{dish_id}/enrichment-jobs", json={"fields": ["texture"]},
...
def test_primary_and_addon_candidate_pools_never_mix():
    ...
    primary = api.get("/v1/meal-classes/BF_CRISP_SAVOURY/dishes",
                      params={"role": "primary"}, headers=HEADERS).json()["items"]
    addon = api.get("/v1/meal-classes/BF_CONDIMENT/dishes",
                    params={"role": "addon"}, headers=HEADERS).json()["items"]
...
def test_relationship_and_cloudinary_reference_are_governed():
    ...
    relation = api.post(f"/v1/dishes/{source}/relationships", headers=HEADERS, json={
        "target_dish_id": target, "relationship": "similar_to", "score": 0.86,
        "confidence": 0.8, "explanation_features": ["fermented_batter", "griddle"],
        "evidence": EVIDENCE,
    })
    ...
    image = api.post(f"/v1/dishes/{source}/images", headers=HEADERS, json={
        "cloudinary_public_id": f"dishes/{source}/abc", "cloudinary_version": 1,
        "secure_url": "https://res.cloudinary.com/demo/image/upload/v1/abc",
        "checksum_sha256": "a" * 64, "source_type": "ai_generated",
        "review_status": "accepted", "is_primary": True,
    })

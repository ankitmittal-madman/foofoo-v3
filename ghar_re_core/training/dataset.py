...
FD-11 (no fabricated/synthetic labels) discipline enforced here, not left to the caller:
  ...
  - `event_type in ('accept', 'like')` -> label 1; `event_type == 'dislike'` -> label 0.
...
_POSITIVE_EVENT_TYPES = {"accept", "like"}
_NEGATIVE_EVENT_TYPES = {"dislike"}
...
def build_labeled_dataset(rows: Iterator[dict[str, Any]], catalogue: Catalogue | None = None) -> list[LabeledRow]:

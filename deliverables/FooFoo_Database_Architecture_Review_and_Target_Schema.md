...
| `festivals` | Governed festival/occasion identity, never household religious identity; `EXT/HYB` `CURRENT` (075) | `id`; `festival_code`; `name`; `region_scope_code`; `calendar_system_code`; `source_id`; `confidence`; `status`; timestamps | Unique code; private table; permanent; service-only context reads through RPC |
| `festival_calendar_occurrences` | Dated regional calendar assertion; `EXT/HYB` `CURRENT` (075) | `id`; `festival_id`; `region_scope_code`; `start_date`; `end_date`; `source_id`; `confidence`; `created_at`; unique festival/region/start/source | Date/region index; private permanent versions; `active_festivals_on(date)` facade |

...
export class SupabaseWeekPlanStore implements WeekPlanStore {
  ...
  async persistWeekPlan(
    ...
    const { data: wp, error: wpErr } = await withTimeout(
      this.db
        ...
        .select("id")
        .single(),
      "weekPlanStore.persistWeekPlan.header",
    );
    ...
    const { error: slotErr } = await withTimeout(
      this.db
        .schema(PUBLIC_SCHEMA)
        .from("plan_slots")
        .upsert(slotRows, { onConflict: "week_plan_id,slot_date,meal_slot" }),
      "weekPlanStore.persistWeekPlan.slots",
    );
...
export class SupabaseOnboardingStore implements OnboardingStore {
  ...
  async isPersonalizationGranted(profileId: string): Promise<boolean> {
    ...
    const { data, error } = await withTimeout(
      this.db
        ...
        .limit(1)
        .maybeSingle(),
      "onboardingStore.isPersonalizationGranted",
    );
  ...
  async persistProfile(row: ProfileRow): Promise<void> {
    const { error } = await withTimeout(
      this.db.schema(PUBLIC_SCHEMA).from("profiles").upsert({
        id: row.id,
      ...
      "onboardingStore.persistProfile",
    );
  ...
  async persistHouseholdMembers(profileId: string, members: HouseholdMemberRow[]): Promise<void> {
    ...
    const { error } = await withTimeout(
      this.db.schema(PUBLIC_SCHEMA).from("household_members").insert(
        members.map((m) => ({
      ...
      "onboardingStore.persistHouseholdMembers",
    );
  ...
  async persistOnboardingSessions(
    ...
    const { error } = await withTimeout(
      this.db.schema(PUBLIC_SCHEMA).from("onboarding_sessions").insert(
        sessions.map((s) => ({
          profile_id: profileId,
      ...
      "onboardingStore.persistOnboardingSessions",
    );
  ...
  async persistUserReState(row: UserReStateRow): Promise<void> {
    const { error } = await withTimeout(
      this.db.schema(RE_ENGINE_SCHEMA).from("user_re_state").upsert({
      ...
      "onboardingStore.persistUserReState",
    );
  ...
  async persistTasteVector(
    ...
    const { error } = await withTimeout(
      this.db.schema(RE_ENGINE_SCHEMA).from("user_taste_vectors").upsert({
        profile_id: profileId,
        class_affinity: classAffinity,
        updated_at: new Date().toISOString(),
      }, { onConflict: "profile_id" }),
      "onboardingStore.persistTasteVector",
    );

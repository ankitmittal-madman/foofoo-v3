...
**Version:** v1.0
**Date:** 2026-08-02
**Placement:** docs/archive/implementation/work-packages/ARCHIVED_WP-11_Launch_Readiness_Remediation_Plan_v1.0.md
...
## 6. Changelog addendum (2026-08-03 session, appended not edited-in-place per this repo's rule against silently revising completion claims)

- **P1 item #5 (catalogue reconciliation) is now resolved, verified live, not just re-derived from code.** `ghar_re_service/data/bundle/manifest.json` (committed under the WP-18 commits, `created_at: 2026-08-03T06:10:53Z`) shows `catalogue_source: "data/source/dishes.xlsx via ghar_re_service.scripts.build_catalogue (real 810-dish catalogue)"`, `dish_count: 810`. The live Fly.io instance (`ghar-re.fly.dev`) was queried directly this session: `GET /v1/meta` → `bundle_version: sha256:07506fd7d963dffb`, matching the committed manifest exactly. `GHAR_RE_SERVICE_URL`/`GHAR_RE_SERVICE_SECRET` are already set as Supabase Edge Function secrets, and the RE's own `/v1/meta` metrics counter (`requests_total: 9, success_total: 8`) shows real production traffic already flowing through the `recommendations` Edge Function → RE → bundle path. **The answer to "which catalogue is the source of truth for live recommendations": the 810-dish bundle built from `dishes.xlsx`, sourced independently of both Postgres `public.dishes` (802 rows, used only by the app's content layer) and `ghar_re.dishes` (39 rows, unused by any current runtime code path).**
- **This session also found the `fly.toml`/README "PREPARED, NOT DEPLOYED" status (written earlier, Phase F) was itself stale** — the app was already deployed (`fly status`: 1 running machine, healthy) roughly an hour before this check. Corrected in both files directly (not noted here as still-open) since they mis-described current live state, not a past decision.
- **New finding, not in the original GPT/Claude-Aug-1 audits:** `diag-re-check` is `ACTIVE` in the live Supabase project (confirmed via `supabase functions list`) but has **no matching source anywhere in this repo's git history** (`git log --all` for the name returns nothing, no `supabase/functions/diag-re-check/` directory exists). This is the same category of drift already flagged for DB migrations elsewhere in this repo (a live artifact with no repo record) — now confirmed for an Edge Function too. Not fixed this session (would need either recovering the deployed function's source from the platform or a Founder decision to redeploy/retire it) — flagged for a follow-up work package.
- **Not executed this session, deliberately: dropping `ghar_re.dishes`.** An initial plan (from a prior conversation this session) assumed it was orphaned dead weight. Direct inspection found real FK-dependent tables in the same schema (`dish_pairing_penalties`, `dish_safety_flags`, `dish_name_synonyms`, KB hero-dish links, household runtime dish refs) plus an active validation script (`907_dish_ontology_validation.sql`) checking it. The broader fact — the entire `ghar_re` Postgres schema is internally consistent but not read by any currently-deployed runtime code (which reads `dishes.xlsx`/YAML directly, never Postgres) — is real, but deciding to retire a live, structurally-connected schema is a Founder decision, not something to execute under a general "clean this up" instruction. Left as an open question, not an action item silently taken.

## 5. Versioning & Placement

v1.0 — initial draft, reconciling an external GPT audit, an earlier same-repo Claude self-audit (Aug 1), and this session's own independent Supabase MCP verification (Aug 2). Supersedes nothing; complements WP-8G (recommendation variety, separately drafted this session). §6 appended 2026-08-03, same version — a changelog addendum recording live-verified corrections, not a content rewrite; a formal v1.1 with corrected P1 status is recommended before this document leaves DRAFT.

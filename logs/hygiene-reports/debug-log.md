...
- Confirmed root causes:
  ...
  - `prospective-user-recommendation-cycle`: Supabase Auth rejected the protected password with `invalid_credentials`; the account UUID and linked production profile still exist.
...
- Structural fixes:
  ...
  - Add a guarded credential-rotation operation that updates only the existing, expected auth user after verifying the Supabase project, UUID, and linked profile; authenticate immediately after rotation.
...
- GitHub re-verification: pending push and protected credential rotation.

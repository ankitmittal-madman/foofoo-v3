...
export default function Home() {
  ...
  const saved = useQuery({ queryKey: ["saved-week", today], queryFn: () => fetchSavedWeek(today), staleTime: 0 });

  useEffect(() => { loadWeeklyPlan().then(setOfflinePlan).catch(() => setOfflinePlan(null)); }, []);
  useFocusEffect(useCallback(() => { saved.refetch(); }, [today]));
  ...
  return <View style={styles.screen} testID="home-screen"><ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.dateRow}><View><Text style={styles.todayLabel}>{t("todayPlan")}</Text><Text style={styles.todayDate}>{new Date(`${today}T12:00:00`).toLocaleDateString(locale === "hi" ? "hi-IN" : "en-IN", { weekday: "long", month: "long", day: "numeric" })}</Text></View><Pressable testID="home-refresh" style={styles.refreshButton} onPress={() => setRefreshNonce((value) => value + 1)}><Text style={styles.refreshText}>↻ {t("refresh")}</Text></Pressable></View>
    <View style={styles.episodeList}>{SLOTS.map((slot) => <MealEpisodeSection key={slot} slot={slot} weekday={weekday} slotDate={today} classCode={plan?.[weekday]?.[slot]} initiallyLocked={locks[weekday]?.[slot] === true} refreshNonce={refreshNonce} />)}</View>

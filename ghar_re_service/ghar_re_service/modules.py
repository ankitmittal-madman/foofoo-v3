...
class _FnModule:
    """Adapts a ghar_re_core.scoring function to the ScoringModule shape.

    Each core function has its own arg pattern; this normalises them.
    """
...
def build_registry() -> list[ScoringModule]:
    ...
    return [
        ...
        # additive, weight 1.0
        _FnModule("prior_boost", None, S.prior_boost, "dish_theta_ctx"),
    ]


def compose_base(
    dish, theta, ctx, config, registry: list[ScoringModule]
) -> tuple[float, list[dict]]:
    """Iterate the registry to compute BASE for one dish AND emit its contributions[].
    Returns (base_total, contributions). base_total is Σ (weight · value), identical by
    construction

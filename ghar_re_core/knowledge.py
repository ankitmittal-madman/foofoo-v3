...
# ---------------------------------------------------------------------------
# CLASS-FIRST COHORT LAYER (WP-15 "Ghar RE v1.1 — Class-Enriched Recommendation").
#
...
# `w_cohort · S_cohort(x;cohort)` term — a term that has existed in the frozen scoring equation
# since Spine v1.0 but, until WP-15, had NO implementation (scoring.score() computed only
# BASE x GAIN_Q15). This revives RE-DOC-03's class-taxonomy/cohort-prior SCIENCE without reviving
# the part of that design the Project Context doc actually retired: the 41 FIXED personas and
# class-first CANDIDATE GENERATION. Here, class-fit is one soft additive score term (never a
# filter, never a candidate-generation gate) and the cohort is matched LIVE from theta
# (home_state + household_type + D2 time_route) — no persona ID is ever stored or looked up.
#
# Extracted once (see data/source/class_first_v1/README — generated by a one-off openpyxl
# script, not hand-authored) into three flat CSVs so the engine never opens the source .xlsx:
#   meal_class_master.csv   (131 rows) - class metadata, not yet consumed (kept for WP-15 §5 future work)
#   class_dish_options.csv  (1050 rows) - dish_name -> meal_class_code, curated
#   cohort_matrix.csv       (2952 rows) - cohort_id -> per-slot/day-type class_mix
#
...
# for unscored dishes, not a fabricated boost. See WP-15 Future/Deferred register for the plan to
# raise coverage (fuzzy/ingredient-based matching, founder-reviewed).
# ---------------------------------------------------------------------------
import csv as _csv
import os as _os
...
def _load_class_first_csv(name):
    """Read one of the three extracted class-first CSVs (see module docstring above) from
    class_first_v1/ underneath ghar_re_core.config.SRC — the SAME config-directory seam every
    other runtime config file uses (respects GHAR_RE_CONFIG_DIR, so this resolves correctly both
    from a checked-out repo and from the service's baked bundle; see RE-DOC-10 §8 / config.py's
    own module docstring on why a path relative to this file would break in a container). Caches
    nothing itself — callers cache into the module-level _DISH_TO_CLASS / _COHORT_ROWS globals so
    the file is parsed once per process, matching config.py's own load-once-and-cache pattern."""
...
# household_type (theta, from hh.q1_household_type) -> keyword(s) to match against a
# Cohort_Matrix_v3 row's free-text household_stage column. Deliberately coarse: this is a
# best-effort live match into a persona-shaped dataset, not a lookup by stored persona ID.
_STAGE_KEYWORDS = {
    "single": ("single",), "flatmates": ("single", "flatmate"),
    "couple": ("couple",), "couple_kids": ("family", "couple"),
    "couple_kids_parents": ("joint", "family"), "joint": ("joint", "family"),
}
# D2 time_route -> the Cohort_Matrix_v3 time_pressure band it best corresponds to.
_TIME_ROUTE_TO_PRESSURE = {"OUTSOURCE": "high", "SIMPLIFY": "medium", "DELEGATE": "low"}


def _best_cohort_row(home_state, household_type, time_route):
    """Best-effort live match of this household into one Cohort_Matrix_v3 row: filter to the
    household's home state, then rank by household_stage keyword overlap + time_pressure band
    match. Returns None if the DB has no cohort rows for this state at all (coverage gap, not
    an error) — the cohort layer contributes nothing for that household, honestly."""
    global _COHORT_ROWS
    if _COHORT_ROWS is None:
        _COHORT_ROWS = _load_class_first_csv("cohort_matrix.csv")
    candidates = [r for r in _COHORT_ROWS if r["state_ut"] == home_state]
    if not candidates:
        return None
    stage_kw = _STAGE_KEYWORDS.get(household_type, ())
    tp_target = _TIME_ROUTE_TO_PRESSURE.get(time_route, "medium")

    def _rank(r):
        stage = (r.get("household_stage") or "").lower()
        tp = (r.get("time_pressure") or "").lower()
        return (1 if any(k in stage for k in stage_kw) else 0) + (1 if tp == tp_target else 0)

    candidates.sort(key=_rank, reverse=True)
    return candidates[0]


def cohort_class_mix(theta, ctx):
    """The set of meal_class_codes this household's best-matching Cohort_Matrix_v3 row actually
    plans for the current slot + day-type (weekday/weekend) — the theta-derived replacement for
    RE-DOC-03's fixed-persona class plan. Empty set if no cohort row matches this home state."""
    home_state = theta["home_state"]["value"]
    household_type = theta["household_type"]["value"]
    time_route = theta["time_route"]["value"]
    row = _best_cohort_row(home_state, household_type, time_route)
    if row is None:
        return set()
    daytype = "weekend" if ctx.get("weekday") in ("Saturday", "Sunday") else "weekday"
    mix = row.get(f"{daytype}_{ctx['slot']}_class_mix") or ""
    return set(c for c in mix.split("|") if c)

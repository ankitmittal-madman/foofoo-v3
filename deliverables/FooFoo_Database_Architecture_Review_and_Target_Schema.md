...
| Private intelligence/control | `re_engine.intent_state`, `household_cadence_state`, `member_fairness_state`; `ml.feature_definitions`, `model_registry`, `experiment_assignments`; `ops.data_sources`, `ai_generation_runs`, `ai_provider_usage_daily`, `ai_dish_enrichment_state`, `coverage_gap_log`, `safety_gate_log` | Live foundations; Groq usage/retry/provenance controls active, while several broader lineage/version controls remain absent |

...
from food_ontology_service.migrate import apply_migrations
...
def test_real_postgres_api_idempotency_resolution_and_role_separation(api):
    ...
    assert (
        create(
            api, payload(addon_name, "TEST_ADDON", "addon"), f"create-addon-{suffix}"
        ).status_code
        == 201
    )
    ...
    primary = api.get(
        "/v1/meal-classes/TEST_PRIMARY/dishes", params={"role": "primary"}, headers=AUTH
    ).json()["items"]
    addon = api.get(
        "/v1/meal-classes/TEST_ADDON/dishes", params={"role": "addon"}, headers=AUTH
    ).json()["items"]
    ...
    invalid = create(
        api, payload(f"Invalid {suffix}", "TEST_ADDON", "primary"), f"invalid-role-{suffix}"
    )
    assert invalid.status_code == 409
    assert invalid.json()["error"] == "meal_class_role_mismatch"


def test_real_postgres_job_claim_completion_and_retry(api, repository):
    suffix = uuid4().hex[:10]
    dish = create(
        api, payload(f"Queued {suffix}", "TEST_PRIMARY", "primary"), f"create-queued-{suffix}"
    ).json()
    for key, fields in (("complete", ["texture"]), ("retry", ["region"])):
        result = api.post(
            f"/v1/dishes/{dish['id']}/enrichment-jobs",
            json={"fields": fields},
...
def test_real_postgres_relationship_image_and_update_are_read_after_write(api):
    suffix = uuid4().hex[:10]
    source = create(
        api, payload(f"Source {suffix}", "TEST_PRIMARY", "primary"), f"source-{suffix}"
    ).json()
    target = create(
        api, payload(f"Target {suffix}", "TEST_PRIMARY", "primary"), f"target-{suffix}"
    ).json()
    updated = api.patch(
        f"/v1/dishes/{source['id']}", headers=AUTH, json={"canonical_name": f"Updated {suffix}"}
    )
    assert updated.status_code == 200
    assert updated.json()["canonical_name"] == f"Updated {suffix}"

    relation = api.post(
        f"/v1/dishes/{source['id']}/relationships",
        headers=AUTH,
        json={
            "target_dish_id": target["id"],
            "relationship": "similar_to",
            "score": 0.8,
            "confidence": 0.85,
            "explanation_features": ["class"],
            "evidence": EVIDENCE,
        },
    )
...
def test_real_postgres_claims_are_exclusive(api, repository):
    suffix = uuid4().hex[:10]
    dish = create(
        api, payload(f"Lease {suffix}", "TEST_PRIMARY", "primary"), f"lease-dish-{suffix}"
    ).json()
    repository.enqueue(dish["id"], "enrich", [f"field-{suffix}-a"], 80, False)
    repository.enqueue(dish["id"], "enrich", [f"field-{suffix}-b"], 70, False)
    first = repository.claim_jobs(f"lease-a-{suffix}", 1)
    second = repository.claim_jobs(f"lease-b-{suffix}", 10)
    ...
    for row in first:
        repository.finish_job(row["id"], f"lease-a-{suffix}", "complete")
    for row in second:
        repository.finish_job(row["id"], f"lease-b-{suffix}", "complete")


def test_migration_checksum_drift_fails_closed(tmp_path):
    changed = tmp_path / "001_changed.sql"
    changed.write_text("SELECT 1;\n", encoding="utf-8")
    with pytest.raises(RuntimeError, match="checksum changed"):
        apply_migrations(DSN, tmp_path)
